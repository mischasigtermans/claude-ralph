...
You may append to `.ralph/learnings.txt` — add process learnings the builder missed.
...
You do not run `git add` or `git commit`. `.ralph/` is gitignored — your edits to `stories.json` and `learnings.txt` stay on disk as local state. If you find `.ralph/` files already tracked in git from a prior mistake, do NOT add to that — leave it for the user to clean up.
